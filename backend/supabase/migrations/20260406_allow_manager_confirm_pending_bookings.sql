...
begin
  ...
  if tg_op = 'UPDATE' and new.status is distinct from old.status then
    if old.status = 'pending' and new.status not in ('proof_submitted', 'confirmed', 'rejected', 'cancelled') then
      raise exception 'Invalid booking transition: % -> %', old.status, new.status;
    end if;

    if old.status = 'pending' and new.status = 'confirmed' and new.payment_status <> 'paid' then
      raise exception 'Pending bookings can only be manually confirmed when marked paid';
    end if;
